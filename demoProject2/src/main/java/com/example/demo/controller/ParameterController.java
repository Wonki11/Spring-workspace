...
@Slf4j // Simple logging facade 4(for) java 약자 --> System.out.pirinln 비슷한 종류
// System 출력에 비해 logging 사용하는게 메모리 부담이 적음 log를 이용한 메세지 출력 시 자주 사용
public class ParameterController {
	
	//GETMAPPING main 
	
	@GetMapping("main") // parm/main 주소로 GET 방식 요청 만듬
	 ...
	 * - 작성된 데이터는 매개 변수(파라미터) 타입이 맞게 형변환이(parse} 자동으로 수행
	 * 
	 * [속성 추가 작성법]
	 ...
	 * 		 한 계층에서 다른 계층으로 데이터 전송을 위해 사용 (단순히 전송만 목적으로 함)
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		//getter setter 이용해서 나머지 정보 가져오기
		//굳이 따로 만들지 않아도 lombok @getter @setter 를 만들어 가져오기 때문에 사용 가능 
